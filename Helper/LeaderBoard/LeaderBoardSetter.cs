...
public class LeaderBoardSetter : MonoBehaviour {
	...
	public static LeaderBoardSetter GetSceneDreamloLeaderboard()
	{
		...
		return go.GetComponent<LeaderBoardSetter>();
	}
	
	public void ClearAllScore()
    {
		StartCoroutine(GetRequest(dreamloWebserviceURL + m_Config.m_LeaderBoardPrivateCode + "/clear"));
	}
	...
	void AddScoreWithPipe(string playerName, int totalScore)
	{
		...
		StartCoroutine(GetRequest(dreamloWebserviceURL + m_Config.m_LeaderBoardPrivateCode + "/add-pipe/" + UnityWebRequest.EscapeURL(playerName) + "/" + totalScore.ToString()));
	}
	
	void AddScoreWithPipe(string playerName, int totalScore, int totalSeconds)
	{
		playerName = Clean(playerName);
		StartCoroutine(GetRequest(dreamloWebserviceURL + m_Config.m_LeaderBoardPrivateCode + "/add-pipe/" + UnityWebRequest.EscapeURL(playerName) + "/" + totalScore.ToString()+ "/" + totalSeconds.ToString()));
	}
	...
	void AddScoreWithPipe(string playerName, int totalScore, int totalSeconds, string shortText)
	{
		...
		StartCoroutine(GetRequest(dreamloWebserviceURL + m_Config.m_LeaderBoardPrivateCode + "/add-pipe/" + UnityWebRequest.EscapeURL(playerName) + "/" + totalScore.ToString() + "/" + totalSeconds.ToString()+ "/" + shortText));
	}
	
	void GetScores()
	{
		highScores = "";
		StartCoroutine(GetRequest(dreamloWebserviceURL + m_Config.m_LeaderBoardPublicCode + "/pipe"));
	}
	
	void GetSingleScore(string playerName)
	{
		highScores = "";
		StartCoroutine(GetRequest(dreamloWebserviceURL + m_Config.m_LeaderBoardPublicCode + "/pipe-get/" + UnityWebRequest.EscapeURL(playerName)));
	}
