...
abstract public class parallaxPlan : MonoBehaviour {
	...
	public float ySpeedMultiplicator = 0f;
	...
	protected void InitParralax () {
		...
		if (m_random == null)
		{
			Debug.LogError("Random should not be null");
		}
	...
	void setTheDistanceMultiplicator() {
		speedMultiplicatorY = -(1f + (distance /(cameraDistancePlan0+Mathf.Abs (distance))));
		speedMultiplicatorY *= ySpeedMultiplicator;
	...
	protected virtual void generateNewSpaceBetweenAssetValue(){
	...
	public void setSpeedOfPlan(float newSpeed, float ySpeed){
		...
		speedSign = (int)Mathf.Sign(speed);
